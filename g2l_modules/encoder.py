...
try:
    ...
    print("Warning: Could not import BEiT models. This is fine if you are not using them.")
    beitv2_base_patch16_448 = None
...
# 优先使用 New_weights 目录（官方下载的纯净模型）
_WEIGHTS_DIR_NEW = "backbones/New_weights"
_WEIGHTS_DIR_OLD = "backbones/weights"

# 默认使用新目录
_WEIGHTS_DIR = _WEIGHTS_DIR_NEW if os.path.exists(_WEIGHTS_DIR_NEW) else _WEIGHTS_DIR_OLD
...
def load(name):
    ...
    if "dinov2" in name:
        # 支持多种格式:
        # 1. dinov2reg_vit_base_14 -> parts: ['dinov2reg', 'vit', 'base', '14']
        # 2. vit_base_patch14_dinov2 -> parts: ['vit', 'base', 'patch14', 'dinov2']
        parts = name.split("_")
        
        if "reg" in parts[0]:  # dinov2reg_vit_base_14 格式
            # parts[2] 是 arch (base/small/large)
            # parts[3] 是 patchsize (14)
            arch = parts[2]
            patchsize = parts[3]
        else:  # 传统格式 vit_base_patch14_dinov2
            arch = parts[1]
            patchsize = parts[2].replace("patch", "")
    else:
        arch, patchsize = name.split("_")[-2], name.split("_")[-1]
    
    if "dinov2" not in name and "beit" not in name:
